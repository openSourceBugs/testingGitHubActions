...
class TestLaserCalculator(BaseTestCase):
    ...
    ENERGY_UNITS = {"kJ":Decimal("1000"),"J":Decimal("1"),"mJ":Decimal(".001"),"uJ":Decimal("0.000001"),"nJ":Decimal("0.000000001"),"pJ":Decimal("0.000000000001")}
    TIME_UNITS={"s":Decimal("1"),"ms":Decimal(".001"),"us":Decimal("0.000001"),"ns":Decimal("0.000000001"),"ps":Decimal("0.000000000001"),"fs": Decimal("0.000000000000001")}
    POWER_UNITS={"kW":Decimal("1000"), "W":Decimal("1"),"mW":Decimal(".001"),"uW":Decimal("0.000001"),"nW":Decimal("0.000000001"),"pW":Decimal("0.000000000001")}
    FREQUENCY_UNITS={"MHz":Decimal("1000000"), "Hz":Decimal("1"), "kHz":Decimal("1000"), "Single":Decimal("1")}
    ...
    def laser_power_calculation(self, time, time_unit, energy=1, power=1, energy_unit="J", power_unit="W", freq=1, hz="Hz"):
        #forumla for calculating peak power of a laser
        val = (energy*power*self.ENERGY_UNITS[energy_unit]*self.POWER_UNITS[power_unit]) /(time *self.TIME_UNITS[time_unit])
    ...
    def test_calculator(self):
        ...
        max_energy = Decimal(5)#just in case we need to test max energy instead of average energy

        #the main testing steps
        peak_power = self.laser_power_calculation(power=max_average_power,power_unit="W", time=pulse_width, time_unit="ps", freq=repetition_rate, hz="kHz")
        self.open("https://www.ophiropt.com/laser--measurement/laser-power-energy-meters/services/peak-power-calculator")#  this is a http url but redirects to https.
        ...
        power = self.find_element(LaserCalculatorPage.peak_power_result).get_attribute("value")
        self.assertEqual('{:.2f}'.format(peak_power), str(power))
        power_density = self.power_density_calculation(peak_power, diameter/10)
        density = self.find_element(LaserCalculatorPage.peak_power_density_result).get_attribute("value")
        self.assertEqual('{:.2f}'.format(power_density), str(density)) #forumla is result *pi * (diameter/2) for density
        ...
        density = self.find_element(LaserCalculatorPage.peak_power_density_result).get_attribute("value")
        self.assertEqual('{:.2f}'.format(peak_power), str(power))
        self.assertEqual('{:.2f}'.format(2*power_density), #TODO this 2 factor for Gaussian should probably be somewhere else
                         density)
